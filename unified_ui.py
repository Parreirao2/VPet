...
class ModernSettingsWindow(SimpleSettingsWindow):
    ...
    def _create_general_tab(self, parent):
        ...
        # Transparency slider with value label
        ...
        # Update label and apply transparency when slider moves
    ...
    def _create_save_tab(self, parent):
        """Create save/load tab with modern styling"""
        # Save frame
        save_frame = ttk.LabelFrame(parent, text="Save Pet", padding=10)
        save_frame.pack(fill=tk.X, padx=10, pady=10)
    ...
    def show_notification(self, title, message, color):
        """Show a notification with modern styling"""
        notification = tk.Toplevel(self.window)
        notification.overrideredirect(True)
        notification.attributes('-topmost', True)
        
        # Position relative to settings window
...
class ModernTooltip:
    ...
    def show_tooltip(self):
        ...
        # Create tooltip window with solid background and high z-index
        self.tooltip_window = tk.Toplevel(self.widget)
        self.tooltip_window.wm_overrideredirect(True)  # No window decorations
        self.tooltip_window.wm_geometry(f"+{x}+{y}")
        self.tooltip_window.lift()  # Bring tooltip to front
...
class ModernButton(tk.Canvas):
    ...
    def __init__(self, parent, text="Button", command=None, width=100, height=30, 
        ...
        # Get parent background color for proper rendering
        # Check if parent is a ttk widget (which doesn't support 'bg' property)
        if hasattr(parent, 'winfo_class') and parent.winfo_class().startswith('T'):
            # For ttk widgets, use the default background color
            parent_bg = COLORS['background']
        else:
            # For regular tk widgets, try to get the background color
            parent_bg = parent.cget('bg') if hasattr(parent, 'cget') else COLORS['background']
        
        # Set up canvas with parent background color
...
class SpeechBubble:
    """Creates speech bubbles for pet interactions"""
    ...
    def _create_bubble(self, message):
        """Create a simple speech bubble using a Toplevel window"""
        ...
        # Position the bubble window near the pet
    ...
    def _update_bubble_position(self):
        ...
        bubble_y = max(0, pet_y - bubble_height - 20)  # Position above pet with margin
    ...
    def _start_position_updates(self):
        """Start periodic updates of bubble position"""
...
# ===== STATUS AND INTERACTION PANELS =====

class CombinedPanel(ttk.Frame):
    """Combined panel that shows both pet stats and interaction buttons"""
    
    def __init__(self, parent, pet_manager, canvas):
        super().__init__(parent)
        self.pet_manager = pet_manager
        self.canvas = canvas
        # Initialize simple UI styles
        ...
        self.stats_visible = True  # Track if stats are visible
        self._stat_widgets = {}  # Initialize the stat widgets dictionary
        ...
        # Define available actions - removed interaction buttons as per Features.txt
        # Only keeping inventory and game hub buttons
        self.actions = {
    ...
    def show_panel(self, x=None, y=None):
        """Show the combined panel at the specified position"""
        ...
        # Create actions frame with modern styling - positioned below stats
    ...
    def hide_panel(self):
        ...
        if self.panel_window:
            ...
            self.stats_frame = None
            self.actions_frame = None
            self._stat_widgets = None  # Clear the widget references
            self.is_visible = False
    
    def toggle_stats_visibility(self):
        """Stats are always visible now, this is kept for compatibility"""
        self.stats_visible = True
    
    def set_stats_visibility(self, visible):
        """Stats are always visible now, this is kept for compatibility"""
        # Always ensure stats are visible
        self.stats_visible = True
        self.stats_container.pack(fill=tk.BOTH, expand=True, padx=15, pady=5)
    
    def create_stat_bar(self, parent, label, value, row, critical_threshold=20):
        """Create a progress bar for a stat with modern styling"""
        # Determine status color based on value
        if value <= critical_threshold:
            status_color = COLORS['error']  # Red for bad
        elif value >= 80:
            status_color = COLORS['success']  # Green for good
        else:
            status_color = COLORS['text']  # Black for neutral
        ...
        # Progress bar style based on value
    ...
    def _update_progress_bar(self, bar, value, critical_threshold):
        ...
        try:
            ...
            # Update the percentage label (it's the last child of the bar's parent)
            for child in bar.master.winfo_children():
                if isinstance(child, ttk.Label):
                    child.config(text=f"{int(value)}%")
        except tk.TclError:
            # Widget no longer exists, just skip the update
            pass
    ...
    def perform_action(self, action_type):
        ...
        if result['message']:
            # Create a speech bubble notification
            if hasattr(self.pet_manager, 'speech_bubble'):
                self.pet_manager.speech_bubble.show_bubble('custom', result['message'])
            else:
                # Fallback to messagebox if speech bubble not available
                messagebox.showinfo("Pet Interaction", result['message'])
        
        # Update stats immediately instead of closing
        self.update_stats_display()
    
    def show_inventory(self):
        """Show the pet's inventory"""
        if self.pet_manager:
            # Hide the panel window before showing inventory
            self.hide_panel()
            self.pet_manager.show_inventory()
    ...
    def show_game_hub(self):
        """Show the game hub"""
        if self.pet_manager:
            # Hide the panel window before showing game hub
            self.hide_panel()
            self.pet_manager.show_game_hub()
...
class SimpleSpeechBubble:
    """A simplified speech bubble for pet interactions"""
    
    def __init__(self, canvas, parent):
        self.canvas = canvas
        self.parent = parent
        self.bubble_window = None
        self.bubble_duration = 3000  # 3 seconds display time
        self._bubble_timer = None
    ...
    def show_bubble(self, response_type, custom_text=None):
        """Show a speech bubble with either a random response or custom text"""
...
class SimpleStatusPanel:
    ...
    def show_panel(self, x=None, y=None):
        ...
        self.stats_frame = tk.Frame(main_frame, bg=COLORS['surface'], padx=15, pady=15, width=350, height=350)
    ...
    def _calculate_poop_chance(self):
        ...
        try:
            if hasattr(self.pet_manager, 'pet_state') and hasattr(self.pet_manager.pet_state, 'poop_system'):
                ...
                # Replicate the logic from check_poop_generation in poop_system.py
                cleanliness = self.pet_manager.pet_state.stats.get_stat('cleanliness') if hasattr(self.pet_manager.pet_state, 'stats') else 100
                adjusted_chance = poop_system.poop_chance * (1 + (100 - cleanliness) / 100)
                
                if poop_system.food_consumed > 0:
                    food_effect = poop_system.food_consumed * poop_system.food_poop_multiplier
